...
//            Если игрок защищается вторым (ослабление игрока)
    ...
    private void checkBoss(Player player, Enemy enemy, boolean turn) {
        if (enemy.getType().equals("Босс")) {
            ...
            if (!turn && player.getMoveStatus() == Entity.MoveStatus.ATTACK) {                            // Игрок атакует, босс регенерировался
                enemy.setCurrentHp(calculateDamage(player, enemy, false) - enemy.getDamage()); // двойной урон
            }
        }
    }
